...
function TagFilter({
  ...
  const { data: fetchedTags } = useUserIdTag(userId);
  ...
  const handleAddTag = async () => {
    ...
    const newTags = { name: newTag, user_id: userId };
    try {
  ...
  return (
    <div className="flex flex-wrap gap-2 mt-4">
      {tags.map(tag => (
        <Button key={tag} onClick={() => onFilterChange(tag)} className={cn("rounded-full", currentFilter === tag ? `${tagColor} text-white` : "text-arom_brown hover:text-arom_brown")}> {tag} </Button>
      ))}
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogTrigger asChild>
          <Button variant="outline" size="sm" className="rounded-full text-arom_brown">
